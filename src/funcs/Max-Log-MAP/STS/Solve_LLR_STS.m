function [LLRs, numb_clipping, lambda_jb, lambda_g]= Solve_LLR_STS(sym_rx, M, H, nVar, mod_func)
    ...
    for cur_num = 0:(power(M, Nt) - 1)
        ...
        for level = Nt:-1:1
            ...
            if (d_i > max_a)
                is_list_break = true;
                break;
                % ееееблан, нахуя из цикла выходишь? просто сохрани текущую
                % сумму по уровню (текущий уровень - 1) и бери другой
                % элемент. Пиздец ты алгоритмист бля. Еще и думай как все
                % хранить в памяти блять. У тебя идет перебор, но с
                % отбрасыванием лишних ветвей. Это поиск в ширину, но с
                % отбрасыванием ветвей епта бляяяяяяяя
            end
        end
