...
cur_Decode_Data_ZF = ones(count_decoded_bits_for_frame, 1)*-1;
cur_Decode_Data_MMSE = ones(count_decoded_bits_for_frame, 1)*-1;
cur_Decode_Data_ML = ones(count_decoded_bits_for_frame, 1)*-1;
cur_Decode_Data_STS = ones(count_decoded_bits_for_frame, 1)*-1;
...
for step = 1:count_do_turbocode_for_frame
    ...
    cur_Decode_Data_ML(1 + (step-1)*L : step*L) = ...
        turbodec(-LLR_ML(1 + (step-1)*M_tc : step*M_tc));
