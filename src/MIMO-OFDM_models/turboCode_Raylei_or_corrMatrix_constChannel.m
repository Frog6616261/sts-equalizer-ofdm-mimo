...
for id_fd = N_used_start.start:N_used_start.end
    H = H_fd_fft(:, :, id_fd);
    tic;
    llr_id = Solve_LLR_ML(info_frame_rx_fd(:, id_fd), M, H, nVar_info, @qammod, 'sum-exp');
    cur_time = cur_time + toc;
...
for id_fd = N_used_start.start:N_used_start.end
    H = H_fd_fft(:, :, id_fd);
    tic;
    [llr_id, clips_id] = Solve_LLR_STS(info_frame_rx_fd(:, id_fd), M, H, nVar_info, @qammod, 'opt');
    cur_time = cur_time + toc;
...
cur_Decode_Data_ML = ones(count_decoded_bits_for_frame, 1)*-100;
