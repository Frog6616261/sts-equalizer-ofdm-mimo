...
path_delay = {[1 12 13], [1 3 9 10]};
path_gain_db = {[0 -8 -23], [0 -7 -15 -17]};
...
pilot_frame_tx_fd = zeros(Nt, Nr, N_FFT);
pilot_frame_tx_td = zeros(Nt, Nr, N_FFT);
...
for i = 1:Nt
    ...
    info_frame_tx_td(i, :) = ifft(info_frame_fd_i).*N_FFT;

    for j = 1:Nr
        pilot_frame_fd_ij = zeros(1, N_FFT);
        pilot_frame_fd_ij([N_used_start.start:N_used_start.end N_used_end.start:N_used_end.end]) = ...
            FFT_pilot_coefs_for_message_antennas(i, j, :);
        pilot_frame_tx_td(i, j, :) = ifft(pilot_frame_fd_ij).*N_FFT;
        pilot_frame_tx_fd(i, j, :) = pilot_frame_fd_ij;
    end
end
...
pilot_frame_out = zeros(Nt, Nr, N_FFT + cp_length); 

for i = 1:Nt
    info_frame_out(i, :) = add_cyclic_prefix(info_frame_tx_td(i, :), cp_length);

    for j = 1:Nr
        pilot_frame_ij = squeeze(pilot_frame_tx_td(i, j, :)).';
        pilot_frame_out(i, j, :) = add_cyclic_prefix(pilot_frame_ij, cp_length);
    end
end
...
h = MIMO_Rayleigh_channel(path_delay, path_gain_db, Nr, Nt); 

% Convolution
info_frame_conv = MIMO_convolution(info_frame_out, h, N_FFT, cp_length);
pilot_frame_conv = zeros(Nr, Nr, N_FFT + cp_length);

for i = 1:Nr
    pilot_symol_out_i = squeeze(pilot_frame_out(:, i, :)); 
    pilot_frame_conv(:, i, :) = MIMO_convolution( ...
        pilot_symol_out_i, h, N_FFT, cp_length);
end


%% Add the AWGN (block "AWGN")
[info_frame_out_channel, nVar_info] = ADD_AWGN_MIMO(info_frame_conv, SNR_dB);
pilot_frame_out_channel = pilot_frame_conv; % for tru channel, without noise


%% Remove cyclic prefix
info_frame_out_channel_without_cp = zeros(Nr, N_FFT);
pilot_frame_out_channel_without_cp = zeros(Nr, Nt, N_FFT);
...
for i = 1:Nr
    ...
    for j = 1:Nt
        pilot_frame_out_channel_without_cp(j, i, :) = ...
            remove_cyclic_prefix(squeeze(pilot_frame_out_channel(j, i, :)), cp_length);
    end
end
...
pilot_frame_rx_fd = zeros(Nr, Nt, N_FFT);

for i = 1:Nr
    info_frame_rx_fd(i, :) = fft(info_frame_out_channel_without_cp(i, :))./N_FFT;
    
    for j = 1:Nt
        pilot_frame_rx_fd(j, i, :) = fft(squeeze(pilot_frame_out_channel_without_cp(j, i, :)))./N_FFT;
    end
end


%% Find channel matrix
H_fd_pilot = get_channel_matrix_by_pilot_frames( ...
    pilot_frame_rx_fd, pilot_frame_tx_fd);
