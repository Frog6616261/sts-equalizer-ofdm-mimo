...
Nt = 2;Nr = 2;
SNRs = -2:1:10;
plt_const_for_snr = 18;

%create result array
result_names = ["BER ZF", "BER MMSE", "BER ML", "EVM ZF", "EVM MMSE", "EVM ML", "SNR dB"];
result = [];
...
%% ZF and MMSE Equalizer training and using
...
QAM_REC_Data_ML = Solve_LLR_ML(info_symbols_out_channel, H, M, nVar, SNR_dB);
QAM_REC_Data_ZF = qamdemod(info_symbols_equalized_ZF, M, OutputType="llr");
QAM_REC_Data_MMSE = qamdemod(info_symbols_equalized_MMSE, M, OutputType="llr");
...
Decode_Data_ML = vitDecUNQUANT(QAM_REC_Data_ML(:));

%% Metrics calculation (blocks "BER" and "EVM")
res_bits_out = bits_out(:);
[~, ber_ML] = biterr(res_bits_out(1:end-traice_depth), Decode_Data_ML(traice_depth+1:end));
...
evm_ML = Compute_MSE(mod_vect, info_symbols_out_channel(:));
evm_ZF = Compute_MSE(mod_vect, info_symbols_equalized_ZF(:));
evm_MMSE = Compute_MSE(mod_vect, info_symbols_equalized_MMSE(:));


%% Fill result 
result = [result; [ber_ZF, ber_MMSE, ber_ML, evm_ZF, evm_MMSE, evm_ML, SNR_dB]];
